...
class MultiHeadAttention(nn.Module):
    ...
    def pad_and_rearrange(self, q_s, k_s, v_s, batch_idxs):
        ...
        output_mask = (~invalid_mask).repeat(n_head, 1).unsqueeze(-1).repeat(1, 1, d_v)
    ...
    def combine_v(self, outputs):
        ...
        if not self.partitioned:
            ...
            outputs1 = torch.transpose(outputs1, 0, 1).contiguous().view(-1, \
                    n_head * d_v1)
            outputs2 = torch.transpose(outputs2, 0, 1).contiguous().view(-1, \
                    n_head * d_v1)
            outputs = torch.cat([
                self.proj1(outputs1),
                self.proj2(outputs2),
    ...
    def __init__(self, embedding,
        ...
        for i in range(num_layers):
            ...
            if d_positional is None:
                ...
                ff = PartitionedPositionwiseFeedForward(d_model, d_ff, \
                        d_positional, \
                        relu_dropout=relu_dropout, \
                        residual_dropout=residual_dropout)
